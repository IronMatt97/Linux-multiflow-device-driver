...
static ssize_t dev_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
   ...
   printk("%s: \nPriority mode = %d\nBlocking mode = %d\nValid bytes (low priority stream) = %d\nValid bytes (high priority stream) = %d\n", MODNAME, highPriority, the_object->blockingModeOn, *off, the_object->valid_bytes_lo, the_object->valid_bytes_hi);
...
static ssize_t dev_read(struct file *filp, char *buff, size_t len, loff_t *off)
{
   ...
   else
   {
      ...
      {
         printk("%s: readop minor %d mod non bloccante bassa prio, provo lock\n", MODNAME, minor);
         ok = mutex_trylock(&(the_object->mutex_lo));
         if (ok == 0)
         {
            printk("%s: RESOURCE BUSY - the requested read (%ld bytes) on minor %d will not be performed.\n", MODNAME, len, minor);
